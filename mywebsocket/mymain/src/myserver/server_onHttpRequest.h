...
public:
    ...
    {   //derived().val=90;
	    ...
	    //myfactory[EindexHtml] = new std::stringstream();
	    //myfactory[EmainHtml]  = new std::stringstream();
...
//#define DINDEXHTML myfactory[Ehtmlpage].as<Tss>()[0]
#define DHTMLPAGE myfactory[Ehtmlpage].as<Tss>()

	    //indexHtml << std::ifstream ("./public/index.html").rdbuf();
	    //*htmlpage[0]<< std::ifstream ("./public/index.html").rdbuf();
	    //*myfactory[Ehtmlpage].as<Tss>()[0]<< std::ifstream ("./public/index.html").rdbuf();
	    //*DINDEXHTML<< std::ifstream ("./public/index.html").rdbuf();
	    *DHTMLPAGE[0]<< std::ifstream ("./public/index.html").rdbuf();
	    
	    
	    //myfactory[Ehtmlpage].as<Tss>()[0]->str().length(); 
	    
        //if (!indexHtml.str().length()) 
        //if (!htmlpage[0]->str().length()) 
	    ...
	    *DHTMLPAGE[1]<< std::ifstream ("./public/main.html").rdbuf();
        ...
        {   
            ...
            {   //res->end(indexHtml.str().data(),indexHtml.str().length());
                //res->end(htmlpage[0]->str().data(),htmlpage[0]->str().length());
