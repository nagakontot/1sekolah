...
namespace jwt {
    ...
    bool verifyPEM(const string& str, const string& b64sig, const string& key, const string& alg) {
        ...
        if (bufkey == nullptr) {
            std::cout<<"\nError verifyPEM=bufkey == nullptr";
            return false;
        }
        ...
        if (EVP_DigestVerifyInit(mdctx, nullptr, evp, nullptr, pkey) != 1) {
            std::cout<<"\nError verifyPEM=(EVP_DigestVerifyInit(mdctx, nullptr, evp, nullptr, pkey) != 1)";
            return false;
        }

        if (EVP_DigestVerifyUpdate(mdctx, str.c_str(), str.length()) != 1) {
            std::cout<<"\nError verifyPEM=(EVP_DigestVerifyUpdate(mdctx, str.c_str(), str.length()) != 1)";
            return false;
        }

        if (EVP_DigestVerifyFinal(mdctx, sig.data(), siglen) != 1) {
            std::cout<<"\nError verifyPEM=(EVP_DigestVerifyFinal(mdctx, sig.data(), siglen) != 1)";
            return false;
        }
    ...
    json decode(const string& jwt, const string& key, const set<string>& alg) {
        try {
            ...
            if (alg.count(theAlg) == 0 && !alg.empty()) {
                std::cout<<"\nError decode=alg.count(theAlg) == 0";
                return {};
            }
            ...
            if (theAlg == "none") {
                ...
                if (signature != calculatedSignature || calculatedSignature.empty()) {
                    std::cout<<"\nError decode=signature != calculatedSignature";
                    return {};
                }
            }
            else {
                if (!verifyPEM(encodedToken, signature, key, theAlg)) {
                    std::cout<<"\nError decode=!verifyPEM(encodedToken, signature, key, theAlg)";
                    return {};
                }
            }
