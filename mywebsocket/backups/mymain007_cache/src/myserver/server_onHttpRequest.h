...
#define INSERTMAP(k)    {   auto lb = mymap.lower_bound(k);\
                            if(lb != mymap.end() && !(mymap.key_comp()(k, lb->first)))\
                            {\
                            }\
                            else\
                            {   mymap.insert(lb, MapType::value_type(k, ++myfilecounter));\
                            }\
                        }
...
//#define Z  new std::stringstream(),
//	    myfactory[Ehtmlpage]    = std::vector<std::stringstream*>({ REP(1,0,0,Z) Z });
//#undef Z
    }
...
*/                
                ...
                {   //already exist
                    std::cout<<"\nres "<<url.c_str()<<" already exist in map, using the cache myfilecounter="<<myfilecounter;
                ...
                {   //insert new
                    ...
                    std::cout<<"\ncreate new res "<<url.c_str()<<", myfilecounter="<<myfilecounter;
