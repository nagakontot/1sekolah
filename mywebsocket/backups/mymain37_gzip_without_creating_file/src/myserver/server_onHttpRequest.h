...
//#define DHTMLPAGE myfactory[Ehtmlpage].as<Tss>()
	    ...
	    *DHTMLPAGE[mymap["/index.html"]]<< std::ifstream ("./public/index.html").rdbuf();
        ...
        //DHTMLPAGE[mymap["/index_gzip.html"]]->clear();
        DHTMLPAGE[mymap["/index_gzip.html"]]->str(compressedData);  //clear up

        if (!DHTMLPAGE[mymap["/index_gzip.html"]]->str().length())  
        {   std::cerr << "Failed to load gzip"<<std::endl;
        }
        else
        {   std::cout<<"Success creating index_gzip.html"<<std::endl;
        }
	    ...
	    *DHTMLPAGE[mymap["/main.html"]]<< std::ifstream ("./public/main.html").rdbuf();
        ...
        {   ///////////////////////////////////////////////////////////////////////////////
            ...
            if (url == "/") 
            {   if(s1=="gzip")
                {   std::cout<<"\nGET: res index_gzip.html is already exist in map[1], myfilecounter=1";
                ...
                {   std::cout<<"\nGET: res index.html is already exist in map[0], myfilecounter=0";
