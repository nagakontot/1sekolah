            ...
            {   // control the contents
                ...
                */
                ...
                {   std::cout<<"\nFAIL, JWT is NOT verified.";
                    if(isgzip)
                    {   res->write((char *) header.data(), header.length());
                        res->end(DHTMLPAGE[mymap["/gzip_index.html"]]->str().data(),DHTMLPAGE[mymap["/gzip_index.html"]]->str().length());
                    }
                    else
...
#undef  X	    
    ...
    int minify(std::string filename)
    ...
    int minify_N_gzip(std::string filename)
    {   std::string fn1("./public/"+filename);
        std::string fn2("/"+filename);
        std::string fn3("/gzip_"+filename);
...
//#define DHTMLPAGE myfactory[Ehtmlpage].as<Tss>()
        ...
        if(isgzip)
        {   minify_N_gzip("index.html");
            minify_N_gzip("main.html");
        }
        else 
        {   minify("index.html");
            minify("main.html");
        ...
        {   ///////////////////////////////////////////////////////////////////////////////
            ...
            if (url == "/") 
            {   if(s1=="gzip" && isgzip)
                {   std::cout<<"\nGET: res gzip_index.html is already exist in map[1], myfilecounter="<<myfilecounter;
                    res->write((char *) header.data(), header.length());
                    res->end(DHTMLPAGE[mymap["/gzip_index.html"]]->str().data(),DHTMLPAGE[mymap["/gzip_index.html"]]->str().length());
                }
                else
