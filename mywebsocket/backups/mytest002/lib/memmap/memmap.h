...
///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        static bool TruncateFile(int nFd, unsigned nFileSize, void **pMemFile)
        {   char cZero = 0;
            if ((lseek(nFd, nFileSize - 1, SEEK_SET) == -1) || (write(nFd, &cZero, 1) == -1)) return false;
...
// //////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        {   ftruncate(nMapFd, nFileSize);
            return mmap(0, nFileSize, PROT_READ|PROT_WRITE,MAP_SHARED, nMapFd, 0);
        }
    };
...
// //////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        static void* Map(unsigned nFileSize, int nMapFd)
        {   ftruncate(nMapFd, nFileSize);
            return mmap(0, nFileSize, PROT_READ|PROT_WRITE,MAP_PRIVATE, nMapFd, 0);
        }
...
//////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        {   if (sFileName.length() > 0) unlink(sFileName.c_str());
        }
    };
...
//////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        {   if(msync(*pMemFile, nFileSize, MS_ASYNC) == -1) return false;
            return true;
        }
    };
...
//////////////////////////////////////////////////////////////////////////////
    ...
    public:
        // syncronize data
        static bool Sync(void **pMemFile, unsigned nFileSize) 
        {   if(msync(*pMemFile, nFileSize, MS_SYNC) == -1)return false;
            return true;
        }
    };
...
//////////////////////////////////////////////////////////////////////////////
    ...
    public:
        // syncronize data
        static bool Sync(void **pMemFile, unsigned nFileSize) 
        {   if(msync(*pMemFile, nFileSize, MS_INVALIDATE) == -1)    return false;
            return true;
        }
    };
/////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        void OpenFile() throw (MemMapFileException)
        {
            ...
            {   std::string sFileName(m_sFileName);
                m_sFileName = "";
                throw MemMapFileException("Cannot open file [%s]",sFileName.c_str());
            }
            m_nMapFd = fd;
        }
        ...
        void* MapFile() throw (MemMapFileException)
        {
            ...
            {   close(m_nMapFd);
                ...
                throw MemMapFileException("Failed memory-mapping file: ",m_sFileName.c_str());
            }
            return m_pMemFile;
        }
        ...
        void Sync() throw (MemMapFileException)
        {
            ...
            {   close(m_nMapFd);
                DeletionPolicy::DeleteFile(m_sFileName);
                throw MemMapFileException("Failed to syncronize data to file : [%s], size[%d]",m_sFileName.c_str(),m_nFileSize);
            }
        ...
        {   // set the file's size
            ...
            {   close(m_nMapFd);
                DeletionPolicy::DeleteFile(m_sFileName);
                throw MemMapFileException("Failed to truncate the file : [%s], size[%d]",m_sFileName.c_str(),m_nFileSize);
            }
            m_nFileSize = nSize;
        }
...
//////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        void Sync()                     {   m_pFile->Sync();}                           // syncronize the data
        virtual ~MemMapFilePtr()        {   m_pFile->DeleteFile();delete m_pFile;}      // destructor
        T& operator*()                  {   return *m_pObj;}                            // operator*
        T& operator*() const            {   return *m_pObj;}                            // operator*
        T* operator->()                 {   return m_pObj;}                             // operator->
        T* operator->() const           {   return m_pObj;}                             // operator ->
    };
