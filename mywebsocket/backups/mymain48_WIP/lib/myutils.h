...
//https://stackoverflow.com/questions/32930479/runtime-error-message-when-using-boostwave
std::string strip_comments(std::string const& input)
{
    ...
    {   position_type   pos;
        ...
        lexer_type it   = lexer_type(input.begin(), input.end(), pos, boost::wave::language_support(boost::wave::support_cpp|boost::wave::support_option_long_long));
...
{   
    ...
    *s = boost::regex_replace(*s, nowhitespace, "");
    ...
    //https://stackoverflow.com/questions/9350863/trimming-internal-whitespace-in-stdstring
    //boost::algorithm::trim_all(*s);
    //boost::trim(*s);
    /////////////////////////////////
    //boost::algorithm::trim(*s);
    //boost::regex pattern{"[[:space:]]+", boost::regex_constants::egrep};    
    //*s = boost::regex_replace(*s, pattern, " ");    //cannot be empty space = ""
}

void minify_html(std::string* s) 
{   //minify_nowhitespace(s);

    //boost::regex nocomments1("<!--(.*?)-->");
    //*s = boost::regex_replace(*s, nocomments1, "");

    //boost::regex nocomments2("(/\\*([^*]|(\\*+[^*/]))*\\*+/)|(//.*)");
    //*s = boost::regex_replace(*s, nocomments2, "");
    ////////////////////////
