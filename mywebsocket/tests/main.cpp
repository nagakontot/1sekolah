...
void testAutobahn() {
    ...
    uS::TLS::Context c = uS::TLS::createContext("misc/ssl/cert.pem",
                                                "misc/ssl/key.pem",
                                                "1234");
    if (!h.listen(6001, c, 0, sslGroup) || !h.listen(6000, nullptr, 0, group)) {
...
void measureInternalThroughput(unsigned int payloadLength, int echoes, bool ssl) {
    ...
    if (ssl) {
        ...
        h.connect("wss://localhost:6000", nullptr);
    } else {
        if (!h.listen(6000)) {
            std::cout << "FAIL: ERR_LISTEN" << std::endl;
            exit(-1);
        }
        h.connect("ws://localhost:6000", nullptr);
    }
...
void testListening() {
    ...
    h.onError([](int port) {
        ...
        case 80:
            ...
            break;
        case 6000:
            std::cout << "Server emits error listening to port 6000 twice" << std::endl;
            break;
        default:
    ...
    h.listen(80);
    if (h.listen(6000)) {
        std::cout << "Server listens to port 6000" << std::endl;
    }
    h.listen(6000);
    h.getDefaultGroup<uWS::SERVER>().close();
    h.run();
...
void testClosing() {
    ...
    h.onConnection([&h, closeMessage](uWS::WebSocket<uWS::SERVER> *ws, uWS::HttpRequest req) {
        ...
        h.connect("ws://localhost:6000", (void *) 2);
    });
    ...
    if (!h.listen(6000)) {
        std::cout << "FAILURE: Cannot listen to port 6000!" << std::endl;
        exit(-1);
    }
    h.connect("ws://localhost:6000", (void *) 1);
...
void testRouting() {
    ...
    h.listen(6000);
    h.connect("ws://localhost:6000/somePathHere", nullptr, {{"sec-websocket-protocol", "someSubProtocolHere"}, {"some-random-header", "someRandomValue"}});
...
void testReusePort() {
    ...
    if (h.listen(6000, nullptr, uS::REUSE_PORT, group1) && h.listen(6000, nullptr, uS::REUSE_PORT, group2)) {
...
void testHTTP() {
    ...
    std::thread t([&expectedRequests]() {
        ...
        // invalid data
        nc = popen("nc localhost 6000 &> /dev/null", "w");
